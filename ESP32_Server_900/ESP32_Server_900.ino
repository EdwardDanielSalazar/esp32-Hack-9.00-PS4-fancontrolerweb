...
#define usbPin 15  // set the pin you want to use for usb control
#else
...
//const char* ssid = "Daniel2";
//const char* password = "Lilidom2019";
//IPAddress ip(192, 168, 101, 10);
...
//String AP_SSID = "PS4_WEB_AP";
//String AP_PASS = "password";
int DS18S20_Pin = 34; ////termometro
float temperature;
int val;//Crea una variable entera
float temp;//Variable de temperatura = temp
int valor;
int g = EEPROM.read(0);
#define LED_PIN         35  ////cooler
int valoreeprom;
const int outputeeprom = 14;
const int output = 2;
//String sliderValue = "0";
String sliderValue = String(EEPROM.read(0));
String sliderValue2 = String(EEPROM.read(0));
...
String getContentType(String filename) {
  ...
  else if (filename.endsWith(".ttf")) return "text/ttf";
  return "text/plain";
  }

const char wifiman_html[] PROGMEM = R"rawliteral(
<!DOCTYPE HTML><html>
<head>
      ...
      <div class="card">
        <form action="/" method="POST">
          <p>
            ...
            <input type="text" id ="ip" name="ip" value="192.168.101.200"><br>
            <label for="gateway">Gateway Address</label>
            <input type="text" id ="gateway" name="gateway" value="192.168.101.1"><br>
...
float getTemp() {
  ...
  //Lee el valor del pin analogo 0 y lo mantiene como val
  if (analogRead(DS18S20_Pin) == 0) {
    val = 1;
  } else {
    val = analogRead(DS18S20_Pin);

  }
  float Temp;
  Temp = log(((40950000 / val) - 10000));
...
void handlePayloads(AsyncWebServerRequest *request) {
  ...
  int payloadCount = 0;
  if (USB_WAIT < 5000) {
    USB_WAIT = 5000; // correct unrealistic timing values
  }
  if (USB_WAIT > 25000) {
    USB_WAIT = 25000;
  }
...
//{
// String htmStr = "<!DOCTYPE html><html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>Admin Panel</title><style>body {    background-color: #1451AE; color: #ffffff; font-size: 14px;  font-weight: bold;    margin: 0 0 0 0.0;    padding: 0.4em 0.4em 0.4em 0.6em;}.sidenav {    width: 140px;    position: fixed;    z-index: 1;    top: 20px;    left: 10px;    background: #6495ED;    overflow-x: hidden;    padding: 8px 0;}.sidenav a {    padding: 6px 8px 6px 16px;    text-decoration: none;    font-size: 14px;    color: #ffffff;    display: block;}.sidenav a:hover {    color: #1451AE;}.main {    margin-left: 150px;     padding: 10px 10px; position: absolute;   top: 0;   right: 0; bottom: 0;  left: 0;}</style></head><body><div class=\"sidenav\"><a href=\"/index.html\" target=\"mframe\">Main Page</a><a href=\"/info.html\" target=\"mframe\">ESP Information</a><a href=\"/fileman.html\" target=\"mframe\">File Manager</a><a href=\"/upload.html\" target=\"mframe\">File Uploader</a><a href=\"/update.html\" target=\"mframe\">Firmware Update</a><a href=\"/wifiman\" target=\"mframe\">Config Editor</a><a href=\"/reboot.html\" target=\"mframe\">Reboot ESP</a><a href=\"/cooler\">Cooler ESP</a></div><div class=\"main\"><iframe src=\"info.html\" name=\"mframe\" height=\"100%\" width=\"100%\" frameborder=\"0\"></iframe></div></table></body></html> ";
// request->send(200, "text/html", htmStr);
//}
...
// request->send(200, "text/html", htmStr);
//}
void coolerhtml(AsyncWebServerRequest *request)
{
  String htmStr = "cooler";
  request->send(200, "text/html", htmStr);
}
...
void setup() {
  ...
  //Serial.begin(115200);
  int y = map(EEPROM.read(0), 0, 100, 195, 0);
  ...
  if (initWiFi()) {
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
      request->send_P(200, "text/html", wifiman_html, processor);
    });
  ...
  server.on("/cooler.html", HTTP_POST, [](AsyncWebServerRequest * request) {
    coolerhtml(request);
  });
  ...
  server.on("/js", HTTP_GET, [](AsyncWebServerRequest * request) {
    request->send(SPIFFS, "/gsap.js", "text/javascript");
  });
  server.on("/js", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  server.on("/slider2", HTTP_GET, [] (AsyncWebServerRequest * request) {
    ...
    if (request->hasParam(PARAM_INPUT)) {
      inputMessage = request->getParam(PARAM_INPUT)->value();
      sliderValue2 = inputMessage.toInt();

      valor = sliderValue2.toInt();
      int y = map(valor, 0, 100, 195, 0);
      analogWrite(LED_PIN, y);
      ledcWrite(ledChannel, y);
      //Serial.println(y);
  ...
  server.on("/guardar", HTTP_GET, [] (AsyncWebServerRequest * request) {
    ...
    //request->send_P(200, "text/plain", readDHTTemperature().c_str());
    request->send(200, "text/plain", valorg().c_str());
